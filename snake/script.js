...
const directionDictionary = {
    ArrowUp: { value: "top", type:'vertical', update: (x, y) => [x, y === 1 ? FIELD_SIZE : y - 1] },
    ArrowRight: { value: "right", type:'horizontal', update: (x, y) => [x === FIELD_SIZE ? 1 : x + 1, y] },
    ArrowDown: { value: "down", type:'vertical', update: (x, y) => [x, y === FIELD_SIZE ? 1 : y + 1] },
    ArrowLeft: { value: "left", type:'horizontal', update: (x, y) => [x === 1 ? FIELD_SIZE : x - 1, y] }
   };

function getElementByCoordites(x, y) {
...
coordinates.forEach((el, index) => {
  const elem = getElementByCoordites(...el);
...
function createMouse() {
  const generateMouse = () => {
    ...
    mouse = getElementByCoordites(x, y);
  };
...
function move() {
  ...
  const newHead = getElementByCoordites(newX, newY);
  ...
  if(newHead.classList.contains('mouse')) {
    ...
    snake.push(getElementByCoordites(lastX, lastY));
    score++;
    input.value = `Ваши очки : ${score}`;
    createMouse();
   }
